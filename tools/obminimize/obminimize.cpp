...
int main(int argc,char **argv)
{
  ...
  for (c=1;;c++) {
    cout << "Iteration " << c << " s1" << endl;
    mol.Clear();
    if (!conv.Read(&mol, &ifs))
      break;
    if (mol.Empty())
      break;
    cout << "Iteration " << c << " s2" << endl;
    if (hydrogens)
      mol.AddHydrogens();
    cout << "Iteration " << c << " s3" << endl;
    if (!pFF->Setup(mol)) {
      cerr << program_name << ": could not setup force field." << endl;
      exit (-1);
    }
    cout << "Iteration " << c << " s4" << endl;
    bool done = true;
    if (sd) {
    ...
    cout << "Iteration " << c << " s6" << endl;

    pFF->GetCoordinates(mol);

    conv.Write(&mol, &cout);
    cout << "Iteration " << c << " s7" << endl;
  } // end for loop
